...
import requests
import asyncio
from api.lastfm import (
    get_artist_albums, get_album_info, get_user_info,
    get_user_top_tracks, get_track_tags, get_recent_tracks
)
    ...
    if 'topalbums' in albums_info:
        ...
        for album in albums[:5]:
            ...
            if 'album' in album_info:
                ...
                image_urls = {img['size']: img['#text'] for img in images if img['#text']}
                if image_urls:
                    image_url = image_urls.get('medium')
                    response = requests.get(image_url)
                    if response.status_code == 200:
                        image_data = BytesIO(response.content)
                        embed = discord.Embed(title=f'Álbum: {album_name}', description=f'Artista: {artist_name}')
...
async def clear_channel(ctx, channel_id: int):
    ...
    try:
        ...
        await ctx.send(f'{len(deleted)} mensagens apagadas do canal {channel.name}.', delete_after=5)
...
async def show_commands(ctx):
    ...
    embed = discord.Embed(title="Comandos Disponíveis", description=commands_description, color=discord.Color.blue())
...
async def fetch_top_tracks(ctx, username: str):
    ...
    if 'toptracks' in top_tracks_info:
        ...
        if not tracks:
            ...
            return
        description = "\n".join([f"{t['name']} por {t['artist']['name']} - {t['playcount']} scrobbles" for t in tracks])
        embed = discord.Embed(title=f'Top Faixas de {username}', description=description, color=discord.Color.purple())
        await ctx.send(embed=embed)
    else:
        await ctx.send(f'Não foi possível encontrar informações para o usuário {username}.')
...
async def fetch_album_tracks(ctx, artist_name: str, *, album_name: str):
    ...
    if 'album' in album_info:
        ...
        if not tracks:
            await ctx.send(f'Nenhuma faixa encontrada para o álbum "{album_name}" de {artist_name}.')
            return
        description = "\n".join([f"{t['name']} - {int(t['duration']) // 60}:{int(t['duration']) % 60:02d}" for t in tracks])
        embed = discord.Embed(title=f'Faixas do álbum "{album_name}"', description=description, color=discord.Color.orange())
        await ctx.send(embed=embed)
    else:
...
async def fetch_common_tracks(ctx, user1: str, user2: str, days: int):
    ...
    if not tracks_user1 or not tracks_user2:
        await ctx.send(f'Não foi possível encontrar faixas para um ou ambos os usuários.')
        return

    # Extrair nome das faixas e artistas de ambos os usuários
    set_user1 = {(track['name'], track['artist']['#text']) for track in tracks_user1}
    set_user2 = {(track['name'], track['artist']['#text']) for track in tracks_user2}

    # Encontrar faixas em comum
    common_tracks = set_user1.intersection(set_user2)

    if not common_tracks:
        await ctx.send(f'Nenhuma faixa em comum encontrada entre {user1} e {user2} nos últimos {days} dias.')
        return

    # Construir a mensagem com as faixas em comum
    description = "\n".join([f"{track[0]} por {track[1]}" for track in common_tracks])
